...
const memories = [
  { id: 1, caption: "Beautiful moments ✨" },
  { id: 2, caption: "Your lovely smile 💕" },
  { id: 3, caption: "Precious memories 🌸" },
  { id: 4, caption: "Forever cherished 💝" },
];
...
const MemoryGallery = () => {
  ...
  return (
    <section className="relative py-24 px-4 mocha-swirl-bg overflow-hidden">
      {/* Decorative elements */}
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
          {memories.map((memory, index) => (
            <div
              key={memory.id}
              className={`polaroid transform transition-all duration-500 cursor-pointer ${
                activeIndex === index ? 'scale-105 z-20' : 'hover:scale-105 hover:-rotate-2'
              }`}
              style={{
        ...
        <p className="text-center mt-12 font-body text-muted-foreground italic">
          (You can add more photos here to make it even more special!)
        </p>
      </div>
    </section>
  );
